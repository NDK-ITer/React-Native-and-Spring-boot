...
public class AuthnService {
    ...
    public boolean JWTIsLogout(String jwt) {
        var userJWT = userJWTRepository.findByToken(jwt);
        if (userJWT == null)
            return false;
        return userJWT.getIsLogout();
    }
    ...
    public HashMap<String, String> Login(LoginModel loginModel, String tokenCode) {
        ...
        if (userJWT == null || userJWT.getExpired().isBefore(LocalDateTime.now())) { // if JWT is not exist or JWT is
                                                                                     // expired
